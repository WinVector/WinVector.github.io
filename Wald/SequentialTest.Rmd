...
We will phrase this as purchasing web-traffic from two sources in parallel.  Each
...
skewing the frame so our decision boundaries are horizontal lines
and we are always stepping up or down as we move to the right (actually it is 
affine transform as it includes a rescaling).


On the skewed graph we always move one to the right on (0,1) and (1,0) observations (which is now
irrelevant as the skewed acceptance lines are horizontal) and either down 1/2 or up 1/2 depending
...
For our example the acceptance rule (in skewed form) is: "run until one of the options is more than k wins ahead of the other and then quit".  The record keeping doesn't even
