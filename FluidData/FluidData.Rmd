...
`cdata` supplies two general operators for fluid data work at database scale (and `Spark` big data scale):
 ...
 1) [`moveValuesToRows*()`](https://winvector.github.io/cdata/reference/rowrecs_to_blocks_q.html): operators centered around `SQL` `cross-join` semantics.  un-pivot, `tidyr::gather()`, and [`cdata::unpivot_to_blocks()`](https://winvector.github.io/cdata/reference/unpivot_to_blocks.html) are special cases of this general operator.
 2) [`moveValuesToColumns*()`](https://winvector.github.io/cdata/reference/blocks_to_rowrecs_q.html): an operator centered around `SQL` `group by` semantics.  pivot, `tidyr::spread()`,  [`cdata::pivot_to_rowrecs()`](https://winvector.github.io/cdata/reference/pivot_to_rowrecs.html), transpose, and one-hot-encoding are special cases of this general operator.
