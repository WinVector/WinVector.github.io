...
library("tint")
# invalidate cache when the tint version changes
...
```{r echo=FALSE, fig.fullwidth = TRUE, fig.cap = "[Boris Artzybasheff illustration](http://www.darkroastedblend.com/2014/01/machines-alive-whimsical-art-of-boris.html)"}
pltImg("wireLoom.png")
```
...
The above operation is a bit exotic and it (and its inverse) already go under number of different names:
  ...
  * [`pivot_to_rowrecs()`](https://winvector.github.io/cdata/reference/pivot_to_rowrecs.html) and [`unpivot_to_blocks()`](https://winvector.github.io/cdata/reference/unpivot_to_blocks.html) (this writeup, basic ["coordinatized data"](http://winvector.github.io/FluidData/RowsAndColumns.html)^[original [here](https://github.com/WinVector/cdata/blob/master/extras/RowsAndColumns.md)])
  * [`blocks_to_rowrecs()`](https://winvector.github.io/cdata/reference/blocks_to_rowrecs_q.html) and [`rowrecs_to_blocks()`](https://winvector.github.io/cdata/reference/rowrecs_to_blocks_q.html) (the more genaral ["fluid data"](https://github.com/WinVector/cdata/blob/master/extras/FluidData.md) operators)

And we are certainly neglecting other namings of the concept.  We find none of these particularly evocative (though [cheatsheets help](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf)), so one purpose of this note will be to teach these concepts in terms of the deliberately verbose ad-hoc terms: `pivot_to_rowrecs()` and `unpivot_to_blocks()`.
...
We will replace the `gather` operation with `unpivot_to_blocks()` and the call will look like the following.
...
`unpivot_to_blocks()` is (under some restrictions) an inverse of `pivot_to_rowrecs()`.


```{r echo=FALSE, fig.fullwidth = TRUE, fig.cap="Figures we will explain later"}
pltImg("gather_spread.png")
```

We find the more verbose naming (and calling interface) more intuitive. So we encourage you to think directly in terms of `unpivot_to_blocks()` as moving values to different rows (in the same column), and `blocks_to_rowrecs()` as moving values to different columns (in the same row).  It will usually be apparent from your problem which of these operations you want to use.
...
The thing to remember: coordinatized data is in cells, and every cell has unique coordinates.  We are going to use this invariant as our enforced [precondition](https://en.wikipedia.org/wiki/Precondition) before any data transform, which will guarantee our data meets this invariant as a postcondition.  I.e., if we restrict ourselves to coordinatized data and exclude wild data, the operations  `pivot_to_rowrecs()` and `unpivot_to_blocks()` become well-behaved and much easier to comprehend. In particular, *they are invertible*. (In math terms, the operators `pivot_to_rowrecs()` and `unpivot_to_blocks()` form a [groupoid](https://en.wikipedia.org/wiki/Groupoid) acting on coordinatized data.)

```{r echo=FALSE, fig.margin = TRUE, fig.cap="[The 15 puzzle: another groupoid](http://www.neverendingbooks.org/the-15-puzzle-groupoid-1)"}
pltImg("15puzzleGroupoid.png")
```
...
```{r sc1to3}
d3from1 <- unpivot_to_blocks(data=d1,
...
In a `unpivot_to_blocks()` operation each row of the data frame is torn up and used to make many rows.  Each of the columns we specify that we want measurements from gives us a new row from each of the original data rows.
...
To make things easier to understand and remember, we introduce another function: `pivot_to_rowrecs()`.

This lets us rework the example of moving from Data Scientist 3's format to Data Scientist 1's:

```{r mvtce}
d1from3 <- pivot_to_rowrecs(data= d3,
                    columnToTakeKeysFrom= 'measurement',
                    columnToTakeValuesFrom= 'value',
                    rowKeyColumns= c('model', 'testset')) %>%
  select(model, depth, testset, AUC, pR2) %>%
...
pivot_to_rowrecs(data= d3damaged,
                    columnToTakeKeysFrom= 'measurement',
                    columnToTakeValuesFrom= 'value',
                    rowKeyColumns= c('model', 'testset'))
```
...
knitr::kable(d3damaged)
knitr::kable(spread(d3damaged, 'measurement', 'value'))
```
...
We feel the methods [`unpivot_to_blocks()`](https://winvector.github.io/cdata/reference/unpivot_to_blocks.html) and [`pivot_to_rowrecs()`](https://winvector.github.io/cdata/reference/pivot_to_rowrecs.html) are easier to learn and remember than abstract terms such as "stack/unstack", "melt/cast", or "gather/spread" and thus are a good way to teach.  Perhaps they are even a good way to document (and confirm) your intent in your own projects.  These terms also anticipate the more general table controlled operators [`rowrecs_to_blocks()`](https://winvector.github.io/cdata/reference/rowrecs_to_blocks_q.html) and [`blocks_to_rowrecs()`](https://winvector.github.io/cdata/reference/blocks_to_rowrecs_q.html).  For more on the general theory please see ["Fluid data reshaping with cdata"](http://winvector.github.io/FluidData/FluidDataReshapingWithCdata.html).
