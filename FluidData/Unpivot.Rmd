...
One great example of this is: the [`sparklyr` package](https://cran.rstudio.com/web/packages/sparklyr/index.html), that allows
`R` to work with big data.  In the following example the table we are working with
is remote, for all we know it could be distributed on a large remote data center.
...
with "tall data" (many rows, fewer columns).  With the [`cata` package](https://winvector.github.io/cdata/) this
transform is easy and performant, as we show below.

```{r unpivot}
library("cdata")

cT <- buildUnPivotControlTable(nameForNewKeyColumn = "measurement",
...
And the reverse operation (often called pivoting) is also easy (and in
fact shares the same control structures).

```{r pivot}
dL <- moveValuesToColumnsN(dTName,
                           keyColumns = "name",
                           controlTable = cT,
                           my_db = sc) %.>%
  dplyr::tbl(sc, .)
...
`cdata` is itself is based on `SQL` aggregations using the `DBI` package, which is why "BB8"'s `NA`
(likely represented on the remote system as a `NULL`) gets changed to a `NaN`.

More on these sort of transforms can be found in the folling notes:
