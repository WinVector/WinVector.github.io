...
This is easy to achieve:^[People have asked, so I will again discuss it.
A lot of the other popular methods for renaming columns
to not appear to work with the current CRAN versions of `dplyr` (0.7.4) and `sparklyr` (0.6.4).  Both `rlang::set_names(d, toupper)`,
and `dplyr::rename(d, HAIR_COLOR=hair_color, SKIN_COLOR=skin_color)` currently throw
when given `Spark` data handles.  It is a topic of some discussion: 
