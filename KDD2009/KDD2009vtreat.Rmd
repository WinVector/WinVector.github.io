...
if(addOtherTargets) {
  ...
  for(yV in c('appetency','upselling')) {
    ...
    treatedTrainMO = prepare(treatmentsCO,
                             ...
                             pruneSig=kddSig, 
                             parallelCluster=cl)
    selvarsO = setdiff(colnames(treatedTrainMO),yV)
    selvarsO <- selvarsO[grep("_catB",selvarsO)]
    treatedTrainMO <- treatedTrainMO[,selvarsO,drop=FALSE]
    colnames(treatedTrainMO) <- paste(yV,colnames(treatedTrainMO),sep='_')
    treatedTrainM <- cbind(treatedTrainM,treatedTrainMO)
    treatedTestO = prepare(treatmentsCO,
                           dTest,
                           pruneSig=kddSig, 
                           parallelCluster=cl)
    treatedTestO <- treatedTestO[,selvarsO,drop=FALSE]
    colnames(treatedTestO) <- paste(yV,colnames(treatedTestO),sep='_')
    treatedTest <- cbind(treatedTest,treatedTestO)
  }
  selvars = setdiff(colnames(treatedTrainM),c(outcomes,'rgroup'))
}
