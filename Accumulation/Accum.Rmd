...
plotTimings <- function(timings) {
  ...
  list(
    ggplot(data=timings,aes(x=nRow,y=time,color=expr)) +
      geom_point(alpha=0.8) + geom_smooth(alpha=0.8),
    ScatterBoxPlot(tsub,'expr','time',title=paste('nRow = ',nmax)) +
      coord_flip()
  )
}
```
...
mkFrameBind <- function(nRow,nCol) {
 bind_rows(replicate(nRow, 
...
mkFrameLDPLY <- function(nRow,nCol) {
 ldply(seq_len(nRow),function(i) data.frame(mkRow(nCol),
                                            stringsAsFactors=FALSE))
}
...
for(nRow in timeSeq) {
  ti <- microbenchmark(
    mkFrameBind(nRow,nCol),
    mkFrameLDPLY(nRow,nCol),
