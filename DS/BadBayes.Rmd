...
runExample <- function(rows,features,trainer,predictor,ndup=0,title) {
  ...
  dTrain$pred <- ifelse(dTrain$predScore>=0.5,yValues[[2]],yValues[[1]])
  ...
  dTest$pred <- ifelse(dTest$predScore>=0.5,yValues[[2]],yValues[[1]])
...
res <- runExample(rows=800,features=400,title='Random Forest ROC plot',
   trainer=function(yName,varNames,yValues,data) {
      formula <- as.formula(paste(yName,paste(varNames,collapse=' + '),
         sep=' ~ '))
...
library(gbm)
set.seed(123525)   # make result more repeatable
res <- runExample(rows=800,features=400,ndup=5,title='Gradient Boost',
                  trainer=function(yName,varNames,yValues,data) {
